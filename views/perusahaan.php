...
<div class="header_table_erp">
	...
	<div  onClick="add('1')" class="header_bottom_table_erp">
		TAMBAH DATA
	</div>
</div>
		...
		<table id="myTable" style="font-size:13px;width:1500px;">		
			<thead>
				<tr onClick=""  class="border_black">
					...
					<th style="width:5%">Bank Cabang</th>
					<th style="width:5%">Akun Bank</th>
					<th style="width:5%">Keterangan</th>
				</tr>
			</thead>
			<tbody>
				<?php $no = 1; ?>
				<?php foreach( $prsh as $row ) : ?>
					<tr>
						<td><?= $no ?></td>
						<td>
...
function update(param){
	$('#tester').html('Loading...');
	$.post('<?php echo $config->base_url();?>selectprsh/'+param+'.html',function(data){
		$('#tester').html(data);
	});
}
...
function action_delete(param){
	$('#tester').html('Loading...');
	$.post('<?php echo $config->base_url();?>hapusprsh/'+param+'.html',function(data){
		$.post('<?php echo $config->base_url();?>prsh/1.html',function(data){
			$('#tester').html(data);
		});
	});
}
function aktif(param){	
	$('#tester').html('Loading...');
	$.post('<?php echo $config->base_url();?>aktifprsh/'+param+'.html',function(data){
		$.post('<?php echo $config->base_url();?>prsh/1.html',function(data){
			$('#tester').html(data);
		});
	});	
...
function nonaktif(param){
	$('#tester').html('Loading...');
	$.post('<?php echo $config->base_url();?>nonaktifprsh/'+param+'.html',function(data){
		$.post('<?php echo $config->base_url();?>prsh/1.html',function(data){
			$('#tester').html(data);
		});
	});		
