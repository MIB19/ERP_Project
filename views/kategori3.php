<div class="header_table_erp">
	...
	<div  onClick="add('6')" class="header_bottom_table_erp">
		TAMBAH DATA
	</div>
</div>
...
$(document).ready(function(){
	...
	function update(param){
		 $('#tester').html('Loading...');
		 $.post('<?php echo $config->base_url();?>selectkat3/'+param+'.html',function(data){
		 	$('#tester').html(data);
		 });
	}
	function aktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>aktifmkat3/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/6.html',function(data){
				$('#tester').html(data);
			});
		});
	}
	function nonaktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>nonaktifmkat3/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/6.html',function(data){
				$('#tester').html(data);
			});
		});
	...
	function action_delete(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>hapusmkat3/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/6.html',function(data){
				$('#tester').html(data);
			});
		});
