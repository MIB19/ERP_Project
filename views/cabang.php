<div class="header_table_erp">
	...
	<div  onClick="add('2')" class="header_bottom_table_erp">
		TAMBAH DATA
	</div>
</div>
		...
		<table id="myTable" style="font-size:13px;width:1500px;">
			<thead>
				<tr onClick=""  class="border_black">
					...
					<th style="width:8%">Nama Cabang</th>
					<th style="width:10%">Alamat</th>
                    <th style="width:8%">No. Telp</th>
                    <th style="width:8%">Fax</th>
                    <th style="width:8%">No. HP</th>
                    <th style="width:10%">Email</th>
                    <th style="width:10%">Nama Bank</th>
                    <th style="width:10%">Bank Cabang</th>
                    <th style="width:10%">Akun Bank</th>
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-center"><?php $st = $row['staktif']; if($st == '1'){?>
							...
							<div onClick="nonaktif('<?= $row['no'];?>')" title="Delete" style="box-sizing:border-box; background-color:red;cursor: pointer;"> Nonaktif
							</div> <?php } ?>
					</td>
					<td><?= $row['pnama'] ?></td>
					<td><?= $row['kode'] ?></td>
...
$(document).ready(function(){
	...
	function update(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>selectcab/'+param+'.html',function(data){
			$('#tester').html(data);
		});
	}
	function aktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>aktifmcab/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>prsh/2.html',function(data){
				$('#tester').html(data);
			});
		});
	}
	function nonaktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>nonaktifmcab/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>prsh/2.html',function(data){
				$('#tester').html(data);
			});
		});
	...
	function action_delete(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>hapusmcab/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>prsh/2.html',function(data){
				$('#tester').html(data);
			});
		});
