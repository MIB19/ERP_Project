<div class="header_table_erp">
    <div class="form_erp_top" onClick="add('<?= $tsjh['no']; ?>/<?= $tsjh['kode']; ?>')" style="cursor: pointer;">Tambah Data</div>
...
<div class="layout-button-riwayat">
	<div style="color:white;padding:10px;overflow-y:auto;">
		...
		<div style="margin-top: 10px;">
            <label>Kode Surat Jalan</span></label>
            <div class="alas_luar">
                <input type="text" value="<?php echo $tsjh['kode']; ?>" class="form_input" disabled>
            </div>
        </div>
		<div style="margin-top: 10px;">
            <label>Perusahaan, Cabang</label>
            <div class="alas_luar">
                <input type="text" value="<?= $tsjh['pnama'];?>             /          <?= $tsjh['cnama']; ?>" class="form_input" disabled>
            </div>
        </div>
		<div style="margin-top: 10px;">
            <label>Customer</label>
            <div class="alas_luar">
                <input type="text" value="<?= $tsjh['cust'];?>" class="form_input" disabled>
            </div>
        </div>
		<div style="margin-top: 10px;">
            <label>Tanggal</label>
            <div class="alas_luar">
                <input type="text" value="<?= date("d / m / Y", strtotime($tsjh['tgl']));?>" class="form_input" disabled>
            </div>
        </div>
		<br/>
...
$(document).ready(function(){
	...
	function add(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url(); ?>addtsjd/'+param+'.html',function(data){
			$('#tester').html(data);
		})
	}
