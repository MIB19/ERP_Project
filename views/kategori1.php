<div class="header_table_erp">
	...
	<div  onClick="add('4')" class="header_bottom_table_erp">
		TAMBAH DATA
	</div>
</div>
...
$(document).ready(function(){
	...
	function update(param){
		 $('#tester').html('Loading...');
		 $.post('<?php echo $config->base_url();?>selectkat1/'+param+'.html',function(data){
		 	$('#tester').html(data);
		 });
	}
	function aktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>aktifmkat1/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/4.html',function(data){
				$('#tester').html(data);
			});
		});
	}
	function nonaktif(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>nonaktifmkat1/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/4.html',function(data){
				$('#tester').html(data);
			});
		});
	...
	function action_delete(param){
		$('#tester').html('Loading...');
		$.post('<?php echo $config->base_url();?>hapusmkat1/'+param+'.html',function(data){
			$.post('<?php echo $config->base_url();?>mbrg/4.html',function(data){
				$('#tester').html(data);
			});
		});
