<div class="form_erp">
	<div class="form_erp_top">Edit Kategori 3</div>
	<div style="float:left;">
		<a onclick="balik('6')" style="color:white;text-decoration: none;">
    ...
    <div class="container">
        <form action="<?php echo $config->base_url();?>updatemkat3.html" class="" method="post">
            <input type="hidden" name="no" value="<?= $mkat3['no']; ?>" required>
            ...
            <div style="margin-top: 10px;">
                ...
                <div class="alas_luar">
                    ...
                    <?php foreach( $prsh as $row1 ) : ?> 
                        <option value="<?= $row1['no'] ?>" <?php if($mkat3['mprsh']==$row1['no']) echo 'selected="selected"'; ?>><?= $row1['nama'] ?></option>
                    <?php endforeach ?>
                    </select>
                </div>
            </div>
            <div style="margin-top: 10px;">
                ...
                <div class="alas_luar">
                    ...
                    <?php foreach( $mcab as $row ) : ?> 
                        <option value="<?= $row['no'] ?>" <?php if($mkat3['mcab']==$row['no']) echo 'selected="selected"'; ?>><?= $row['nama'] ?></option>
                    <?php endforeach ?>
                    </select>
                </div>
            </div>
            <div style="margin-top: 10px;">
                ...
                <div class="alas_luar">
                    ...
                    <?php foreach( $mkat2 as $row2 ) : ?> 
                        <option value="<?= $row2['no'] ?>" <?php if($mkat3['mkat2']==$row2['no']) echo 'selected="selected"'; ?>><?= $row2['nama'] ?></option>
                    <?php endforeach ?>
                    </select>
                </div>
            </div>
            <div style="margin-top: 10px;">
                <label>Nama</label>
                <div class="alas_luar">
                    <input type="text" name="nama" class="form_input" value="<?= $mkat3['nama']; ?>" minlength="1" required autofocus>
                </div>
            </div>
            <div style="margin-top: 10px;">
                <label>Keterangan<span class="required">*</span></label>
                <div class="alas_luar">
                    <textarea class="form_input" rows="3" name="ket"><?= $mkat3['ket']; ?></textarea>
                </div>
            </div>
